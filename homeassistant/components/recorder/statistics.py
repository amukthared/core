...
def _fetch_metadata(session, statistic_id: str):
    return get_instance(hass).statistics_meta_manager.get(session, statistic_id)

def _get_time_periods(start_time, end_time, tail_only, oldest_stat, oldest_5_min_stat):
    # [All the logic related to calculating head_start_time, head_end_time, etc. goes here]
    # ...

def _get_max_mean_min_results(session, time_periods, metadata_id, types):
    return _get_max_mean_min_statistic(
                session,
                *time_periods,
                metadata_id,
                types,
...
def _calculate_change(session, start_time, time_periods, oldest_stat, tail_only, metadata_id):
    ...
    return None if oldest_sum is None or newest_sum is None else newest_sum - oldest_sum

def _convert_result_units(result, metadata, state, units):
...
def statistic_during_period(
    ...
    with session_scope(hass=hass, read_only=True) as session:
        metadata = _fetch_metadata(session, statistic_id)
        if not metadata:
            return result
        ...
        time_periods = _get_time_periods(start_time, end_time, tail_only, oldest_stat, oldest_5_min_stat)
        
        if not types.isdisjoint({"max", "mean", "min"}):
            result = _get_max_mean_min_results(session, time_periods, metadata_id, types)

        if "change" in types:
            result["change"] = _calculate_change(session, start_time, time_periods, oldest_stat, tail_only, metadata_id)

    state = hass.states.get(statistic_id)
    return _convert_result_units(result, metadata, state, units)
